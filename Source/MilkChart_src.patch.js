...
MilkChart = new Class({
    ...
    initialize: function(el, options) {
        ...
        this.colNames = [];
        this.rowNames = [];
		this.rowCount = this.options.data.colNames.length;
		// Hacky, oh the shame!
...
/*        this.ctx.fillText(this.options.title, this.bounds[0].x + (this.bounds[1].x - this.bounds[0].x)/2, titleHeight, this.chartWidth);*/
	...
	if(this.ctx.fillText) {
		...
		this.ctx.translate(this.options.title, this.bounds[0].x + (this.bounds[1].x - this.bounds[0].x)/2, titleHeight, this.chartWidth);
		this.ctx.mozDrawText(this.options.title); // for FF3-
		this.ctx.translate( (0-this.bounds[0].x + (this.bounds[1].x - this.bounds[0].x)/2), (0-(titleHeight)) );
	}
		...
		for (i = 0; i < this.chartLines; i++) {
			...
			if (this.options.showValues) {
				if(this.ctx.fillText) {
					...
					this.ctx.translate(this.bounds[0].x - 8, lineY + 3);
					this.ctx.mozDrawText(String(lineValue)); // for FF3-
					this.ctx.translate( (0-(this.bounds[0].x - 8)), (0-(lineY + 3)) );
				}
			}
...
MilkChart.Column = new Class({
    ...
    *********************************/
    ...
    getData: function() {
		this.colNames = this.options.data.colNames;
		if (this.options.useFooter) {
		    this.rowNames = this.options.data.rowNames;
		}
		this.options.data.rows.each(function(row) {
		    var dataRow = [];
        ...
        this.rows.each(function(row) {
			...
			if (this.options.showRowNames) {
				...
				if(this.ctx.fillText) {
					...
					this.ctx.translate(rowOrigin.x+(this.rowWidth/2), this.bounds[1].y+(this.rowPadding/2));
					this.ctx.mozDrawText(this.rowNames[rowNameID]); // for FF3-
					this.ctx.translate( (0-(rowOrigin.x+(this.rowWidth/2))), (0-(this.bounds[1].y+(this.rowPadding/2))) );
				}
			}
    ...
    drawKey: function() {
		...
		if(this.ctx.fillText) {
			...
			this.ctx.translate(this.keyBounds[0].x + textMarginLeft, keyOrigin+8);
			this.ctx.mozDrawText(item); // for FF3-
			this.ctx.translate( (0-(this.keyBounds[0].x + textMarginLeft)), (0-(keyOrigin+8)) );
		}
...
MilkChart.Bar = new Class({
    ...
    drawValueLines: function() {
        ...
        for (i=0;i<this.chartLines;i++) {
			...
			this.ctx.translate(lineX, this.bounds[1].y + 14);
			this.ctx.mozDrawText(String(lineValue)); // for FF3-
			this.ctx.translate( (0-(lineX)), (0-(this.bounds[1].y + 14)) );
		}
    ...
    draw: function() {
		...
		if(this.ctx.fillText) {
			...
			this.ctx.translate(rowOrigin.x-(colWidth/2), rowOrigin.y-(this.rowPadding/2));
			this.ctx.mozDrawText(this.rowNames[rowNameID]); // for FF3-
			this.ctx.translate( (0-(rowOrigin.x-(colWidth/2))), (0-(rowOrigin.y-(this.rowPadding/2))) );
		}
...
MilkChart.Line = new Class({
    ...
    *********************************/
    ...
    getData: function() {
		this.rows = [];
		this.options.data.colNames.each(function(item) {
		   this.colNames.push(item);
		   this.rows.push([]);
		}.bind(this));
		if (this.options.useFooter) {
		    this.rowNames = this.options.data.rowNames;
    ...
    drawKey: function() {
		...
		if(this.ctx.fillText) {
			...
			this.ctx.translate(this.keyBounds[0].x + 30, keyOrigin+5);
			this.ctx.mozDrawText(item); // for FF3-
			this.ctx.translate( (0-(this.keyBounds[0].x + 30)), (0-(keyOrigin+5)) );
		}
    ...
    __drawRowLabels: function() {
		...
		if(this.ctx.fillText) {
			...
			this.ctx.translate(origin.x+rowCenter, this.bounds[1].y+(this.rowPadding/2));
			this.ctx.mozDrawText(this.rowNames[index]); // for FF3-
			this.ctx.translate( (0-(origin.x+rowCenter)), (0-(this.bounds[1].y+(this.rowPadding/2))) );
		}
    ...
    *********************************/
    ...
    draw: function() {
		...
		if(this.ctx.fillText) {
			...
			this.ctx.translate(center.x + x, center.y + y);
			this.ctx.mozDrawText(percent + "%"); // for FF3-
			this.ctx.translate( (0-(center.x + x)), (0-(center.y + y)) );
		}
            }
            arcStart += item[1];
        }.bind(this));
    ...
    drawKey: function() {
        ...
        this.rowNames.each(function(item) {
			...
			this.ctx.translate(this.keyBounds[0].x + 14, keyOrigin+8);
			this.ctx.mozDrawText(item); // for FF3-
			this.ctx.translate( (0-(this.keyBounds[0].x + 14)), (0-(keyOrigin+8)) );
		}
